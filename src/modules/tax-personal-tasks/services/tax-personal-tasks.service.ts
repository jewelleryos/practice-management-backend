...
// `t` = tax_personal_tasks. Used by both list and getById.
const ROW_SELECT = `
...
const VISIBLE_PREDICATE = `(
  t.created_by = $1
  OR EXISTS (
    SELECT 1 FROM tax_personal_task_followers f
...
export const personalTaskService = {
  ...
  async list(actingUser: AuthUser, q: ListPersonalTasksQuery): Promise<PersonalTaskListResponse> {
    ...
    if (q.follower_id) {
      ...
      where.push(
        `EXISTS (SELECT 1 FROM tax_personal_task_followers f2
  ...
  async getById(actingUser: AuthUser, id: string): Promise<PersonalTaskDetail> {
    ...
    if (result.rows.length === 0) {
      throw new AppError(personalTaskMessages.NOT_FOUND, HTTP_STATUS.NOT_FOUND)
    }
  ...
  // Returns the task's created_by (handy for callers). Used by every write.
  ...
  // Validate + normalise a follower id list: dedupe, drop the creator (they see the
  // task via created_by, never as a follower), and confirm each is a real, active
  // member. Returns the cleaned id list.
  async normaliseFollowers(followerIds: string[], creatorId: string): Promise<string[]> {
    ...
    if (result.rows.length !== ids.length) {
      throw new AppError(personalTaskMessages.FOLLOWER_NOT_FOUND, HTTP_STATUS.BAD_REQUEST)
    }
    return ids
  },
  ...
  async create(actingUser: AuthUser, data: CreatePersonalTaskRequest): Promise<PersonalTaskDetail> {
    ...
    try {
      ...
      const inserted = await client.query(
        `INSERT INTO tax_personal_tasks (title, description, status, due_date, created_by)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id`,
      ...
      await placeAtTopOfColumn(client, BOARD_ORDER_TABLES.taxPersonalTasks, taskId)

      for (const memberId of followerIds) {
        await client.query(
          `INSERT INTO tax_personal_task_followers (task_id, member_id) VALUES ($1, $2)`,
          [taskId, memberId],
        )
      }
  ...
  async update(
    ...
    if (sets.length > 0) {
      params.push(id)
      await db.query(
        `UPDATE tax_personal_tasks SET ${sets.join(', ')} WHERE id = $${params.length}`,
        params,
      )
    }
  ...
  // ── CHANGE STATUS (board drag) — any viewer may move it, no completion rules ──
  async changeStatus(
    actingUser: AuthUser,
    id: string,
    status: TaskStatus,
    ...
    await this.assertVisible(actingUser, id)
    await db.query(`UPDATE tax_personal_tasks SET status = $1 WHERE id = $2`, [status, id])
    return this.getById(actingUser, id)
  },
  ...
  async setFollowers(
    ...
    try {
      await client.query('BEGIN')
      await client.query(`DELETE FROM tax_personal_task_followers WHERE task_id = $1`, [id])
      ...
      await client.query(`UPDATE tax_personal_tasks SET updated_at = NOW() WHERE id = $1`, [id])
      await client.query('COMMIT')
    } catch (error) {
      await client.query('ROLLBACK')
      throw error
    ...
    // The caller must still be able to see the task after the change (they are the
    // creator, or they kept themselves as a follower). If a follower removed
    // themselves, getById 404s — that's fine, the route surfaces it.
    return this.getById(actingUser, id)
  },
  ...
  async addNote(
    ...
    const inserted = await db.query(
      `INSERT INTO tax_personal_task_notes (task_id, body, created_by)
  ...
  async updateNote(
    ...
    const updated = await db.query(
      `UPDATE tax_personal_task_notes SET body = $1
       WHERE id = $2 AND task_id = $3 AND is_deleted = FALSE
       RETURNING id`,
      [data.body, noteId, id],
    )
    if (updated.rows.length === 0) {
      throw new AppError(personalTaskMessages.NOTE_NOT_FOUND, HTTP_STATUS.NOT_FOUND)
    }
    const notes = await this.fetchNotes(id)
    return notes.find((n) => n.id === noteId)!
  },

  // ── FOR route: members that can be added as followers (tax-practice members) ──
  async memberOptions(): Promise<PersonalTaskMemberOption[]> {
    const result = await db.query(
      ...
      [DEPARTMENTS.TAX_PRACTICE],
    )
    return result.rows as PersonalTaskMemberOption[]
  },

  // ── BOARD REORDER ──
  // Move a card within its own status column. assertVisible is the module's
  // creator-or-follower guard; it does not return the status, so that is read
  // separately (the row is already proven visible by then).
  async reorderOnBoard(
    ...
    const row = await db.query(
      `SELECT status FROM tax_personal_tasks WHERE id = $1 AND is_deleted = FALSE`,
      [id],
    )
    ...
    const position = await repositionOnBoard(
      BOARD_ORDER_TABLES.taxPersonalTasks,
      id,
      row.rows[0].status,
      afterId,
    )
