...
export const PERMISSIONS = {
  ...
  // Annual Review (tax_practice) — the ASIC annual-review tracker. A SINGLE
  // permission gates the whole page: see the list, change a row's status, edit its
  // notes. There is NO create code — rows are generated by the system from each
  // client's incorporation date, never created by a member. Next 100-block after
  // TAX_PERSONAL_TASK (1301).
  ANNUAL_REVIEW: {
    ACCESS: 1401,
  },
  MORTGAGE_CLIENT: {
...
export const PERMISSION_MODULES: PermissionModule[] = [
  ...
  },
  {
    ...
    label: 'Annual Review',
    department: DEPARTMENTS.TAX_PRACTICE,
    // One permission: access to the annual-review page (view the list, change a
    // row's status, edit its notes). No CREATE/VIEW split — the system generates
    // every row from the client's incorporation date, so there is nothing for a
    // member to create.
    permissions: [
      { code: PERMISSIONS.ANNUAL_REVIEW.ACCESS, action: 'ACCESS', label: 'Access annual review', requires: [] },
    ],
  },
  {
    key: 'clients',
